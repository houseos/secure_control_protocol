...
class ScpMessageSender {
  ...
  static const int DISCOVER_TIMEOUT = 30;
  static const int NVCN_TIMEOUT = 30;
  static const int NEW_PASSWORD_TIMEOUT = 30;
  ...
  static Future<http.Response> sendDiscoverHello(String ip) async {
    Uri url = Uri.http(
      '${ip}:${PORT}',
      '/secure-control/discover-hello',
    ...
    return await http
        ...
        .catchError((e) {
      return http.Response('failed', 404);
    });
  }
  ...
  static sendNewPassword(ScpDevice device) async {
    ...
    Uri url = Uri.http('${device.ipAddress}:$PORT', '/secure-control', {
      'nonce': '${scpJson.encryptedPayload.base64Nonce}',
      'payload': '${scpJson.encryptedPayload.base64Data}',
      'payloadLength': '${scpJson.encryptedPayload.dataLength}',
      'mac': '${scpJson.encryptedPayload.base64Mac}',
  ...
  static Future<ScpStatus> sendWifiConfig(
  ...
  static Future<ScpStatus> sendRestart(ScpDevice device) async {
    // get NVCN
    print('Fetching NVCN');
    var nvcnResponse = await fetchNVCN(device);
    if (nvcnResponse == null) {
  ...
  static Future<ScpStatusMeasure> sendMeasure(
    ...
    if (measureResponse.statusCode == 200) {
      ...
      if (parsedResponse.isValid()) {
        if (parsedResponse.getResult() == ScpStatus.RESULT_SUCCESS &&
            action == parsedResponse.getAction()) {
          print('Successfully measured $action: ${parsedResponse.getValue()}.');
